...
function extractPackageJsonFromReadable(readable: NodeJS.ReadableStream, pkgFilename: string): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    readable.on('error', (err) => {
      console.log('readable', err.message);
      reject(err);
    }).pipe(extract);
  });
}
...
async function fetchPackageJsonFromTarballStream(fileStream: NodeJS.ReadableStream): Promise<any> {
  ...
  console.log('wait');
  await wait();
  console.log('extractPackageJsonFromReadable');
  /**
...
export class CostaRuntime {
  ...
  async fetchByStream(stream: NodeJS.ReadableStream): Promise<PluginPackage> {
    ...
    let filename = path.join(fileDir, 'pkg.tgz');
    console.log('save file form stream to', filename);
    ...
    console.log('fetchPackageJsonFromTarball', filename);
