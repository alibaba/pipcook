...
const defineTextLSTMModel: ModelDefineType = async (data: CsvDataset, args: ModelDefineArgsType): Promise<TfJsLayersModel> => {
  const { recoverPath } = args;
  const { charsetLength, maxLineLength } = data.metadata;

  const model = tf.sequential();
  model.add(tf.layers.embedding({
    inputDim: charsetLength,
    outputDim: 100,
  ...
  model.add(tf.layers.dense({
    units: Math.ceil(charsetLength / 2),
    activation: 'relu',
