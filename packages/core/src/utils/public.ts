...
export function unZipData(filePath: string, targetPath: string) {
  return new Promise((resolve, reject) => {
    extract(filePath, { dir: targetPath }, (err: Error) => {
      if (err) {
        reject(err);
      } else {
        resolve();
 ...
 */
export async function convertPascal2CocoFileOutput(
  files: string[],
  targetPath: string
) {
  const cocoJson: any = {
  ...
  const xmlJsonPromiseArr: any = [];

  files.forEach((item) => {
    xmlJsonPromiseArr.push(parseAnnotation(item));
  });

  await Promise.all(xmlJsonPromiseArr).then((xmlJsonArr) => {
    xmlJsonArr.forEach((xmlJson, xmlJsonIndex) => {
      const imageItem: any = {
        license: 1,
      ...
      if (xmlJson.annotation.size && xmlJson.annotation.size[0]) {
        imageItem.width = parseInt(xmlJson.annotation.size[0].width[0], 10);
        imageItem.height = parseInt(xmlJson.annotation.size[0].height[0], 10);
      }
      cocoJson.images.push(imageItem);
      if (!(
        xmlJson.annotation
        && xmlJson.annotation.object
        && xmlJson.annotation.object.length > 0
      )) {
        continue;
      }
      xmlJson.annotation.object.forEach((item: any) => {
        ...
        const cocoItem: any = {
          ...
          image_id: xmlJsonIndex,
          category_id: id,
          segmentation: [],
          iscrowd: 0
        };
        if (item.bndbox && item.bndbox[0]) {
          const width = parseInt(item.bndbox[0].xmax[0], 10)
            - parseInt(item.bndbox[0].xmin[0], 10);
          const height = parseInt(item.bndbox[0].ymax[0], 10)
            - parseInt(item.bndbox[0].ymin[0], 10);
          cocoItem.bbox = [
            parseInt(item.bndbox[0].xmin[0], 10),
            parseInt(item.bndbox[0].ymin[0], 10),
            width,
