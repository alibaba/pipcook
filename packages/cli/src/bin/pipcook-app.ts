...
program
  ...
  .action(async (srcPath) => {
    ...
    pipelines.forEach(({ id, namespace }) => {
      console.info(`(${namespace.module}.${namespace.method}) > https://pipboard.vercel.app/#/pipeline/info?pipelineId=${id}`);
    });
  });
...
program
  ...
  .action(async (srcPath) => {
    ...
    jobs.forEach((job) => {
      ...
      console.info(`  pipeline: https://pipboard.vercel.app/#/pipeline/info?pipelineId=${job.pipelineId}`);
