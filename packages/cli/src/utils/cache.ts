 ...
 */
export const fetchWithCache = async (cacheDir: string, url: string, target: string, enableCache: boolean, isCopy = false): Promise<void> => {
  ...
  if (enableCache) {
    if (await fs.pathExists(cachePath)) {
      return await isCopy ? fs.copy(cachePath, target) : fs.symlink(cachePath, target);
    }
    debug('cache missed');
  }
  ...
  debug(`copy/link ${cachePath} to ${target}`);
  return isCopy ? fs.copy(cachePath, target) : fs.symlink(cachePath, target);
};
