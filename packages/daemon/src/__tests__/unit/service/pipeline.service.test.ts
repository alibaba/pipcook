...
test('should fech plugins', async (t) => {
  ...
  mockPlugin.stubs.name = 'mockDataCollect';
  mockPlugin.stubs.status = 1;

  const mockPipeline = createStubInstance<Pipeline>(Pipeline);
  mockPipeline.stubs.dataCollectId = 'mockDataCollectId';
  mockPipeline.stubs.dataCollect = 'mockDataCollect';
  mockPipeline.stubs.dataCollectParams = {};
  const mockFindOne = pluginRepository.stubs.findOne.resolves(mockPlugin);
  const mockFetchFromInstalledPlugin = pluginService.stubs.fetchFromInstalledPlugin.resolves({} as any);
  const plugins = await pipelineService.fetchPlugins(mockPipeline);
...
test('fech plugins but plugin not installed - id is null', async (t) => {
  ...
  await t.throwsAsync(pipelineService.fetchPlugins({
    dataCollectId: '',
...
test('fech plugins but plugin not installed - plugin status error', async (t) => {
  ...
  await t.throwsAsync(pipelineService.fetchPlugins({
    dataCollectId: 'mockDataCollectId',
    dataCollect: 'mockDataCollect',
    dataCollectParams: {}
  } as any));
